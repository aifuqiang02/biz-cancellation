...
class AutomationEngine(QObject):
    ...
    # 信号定义
    automation_step_completed = pyqtSignal(str, bool, str)
    message_processed = pyqtSignal(dict, bool)
    ...
    def start(self, config: Dict[str, Any]) -> bool:
        """开始自动化"""
        if self.is_running:
            self.logger.warning("SNAMR自动化已经在运行中")
            return False

        try:
            self.config.update(config)
            self.logger.info(f"启动SNAMR自动化，配置: {self.config}")
            ...
            self.automation_step_completed.emit("启动", True, "SNAMR自动化已启动")
            return True

        except Exception as e:
            self.logger.error(f"启动SNAMR自动化失败: {e}")
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            self.logger.info("SNAMR自动化已停止")
            self.automation_step_completed.emit("停止", True, "SNAMR自动化已停止")
            return True

        except Exception as e:
            self.logger.error(f"停止SNAMR自动化失败: {e}")
            return False
    ...
    def _schedule_next_round(self) -> None:
        """安排下一轮数据采集"""
        if not self.is_running:
            self.logger.info("SNAMR自动化已停止，不安排下一轮")
            return

        # 计算随机间隔（20-30分钟）
        min_interval = self.config.get('round_interval_min', 1200)
        max_interval = self.config.get('round_interval_max', 1800)
    ...
    def manual_check(self) -> None:
        """手动触发数据采集（用于测试）"""
        if not self.is_running:
            self.logger.warning("SNAMR自动化未运行，无法执行手动采集")
            return
