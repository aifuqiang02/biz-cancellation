...
class BrowserController(QObject):
    ...
    def _run_js_sync(self, code: str, callback: Callable) -> None:
        ...
        self.logger.debug(f"[_run_js_sync] 开始执行JS: {code[:80]}...")
        try:
            # 直接执行代码，使用PyQt5的runJavaScript回调
            def handle_result(result):
                try:
                    self.logger.debug(f"[_run_js_sync] JS返回结果: type={type(result)}, value={result}")
                    if result is None:
                        self.logger.warning(f"[_run_js_sync] JS返回None，code: {code[:50]}...")
                        callback([])
                    elif isinstance(result, dict) and 'error' in result:
                        self.logger.error(f"[_run_js_sync] JS执行错误: {result['error']}")
                        callback([])
                    else:
                        self.logger.debug(f"[_run_js_sync] JS执行成功，返回类型: {type(result)}")
                        callback(result)
                except Exception as e:
                    self.logger.error(f"[_run_js_sync] 处理JS结果失败: {e}")
                    callback([])

            self.web_view.page().runJavaScript(code, handle_result)

        except Exception as e:
            self.logger.error(f"[_run_js_sync] 同步JS执行失败: {e}")
            callback([])
    ...
    def send_private_reply(self, reply_text: str, callback: Callable[[bool], None]) -> None:
        ...
        self._run_js_code(f"window.MessageHelpers.sendPrivateMessageReply({reply_json})", callback)
